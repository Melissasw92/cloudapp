...
const subnetIds = vpc.id.apply(id =>
  aws.ec2.getSubnets({
...
const apiSg = new aws.ec2.SecurityGroup("api-sg", {
  ...
  ingress: [
    ...
    // uncomment this if you want SSH from your IP for troubleshooting:
    // { protocol: "tcp", fromPort: 22, toPort: 22, cidrBlocks: ["YOUR.IP.ADDR.ESS/32"] },
  ],
...
const al2Ami = aws.ec2.getAmiOutput({
  mostRecent: true,
...
const api = new aws.ec2.Instance("api", {
  ...
  userData,
  associatePublicIpAddress: true, // make sure it’s reachable from the internet
  tags: { Name: "api" },
});
 ...
 * 6) Outputs
 * ========================= */
export const bucket = site.bucket;
const region = aws.getRegionOutput().region;
export const websiteUrl = pulumi.interpolate`http://${site.bucket}.s3-website-${region}.amazonaws.com`;
